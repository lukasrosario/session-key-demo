...
import { bundlerClient, getUserOpHashForMint } from "./util/getUserOpHashForMint";
...
function App() {
  ...
  const {data: userOpReceipt} = useQuery({queryKey: ['opReceipt', userOpHash], queryFn: () => bundlerClient.getUserOperationReceipt({hash: userOpHash}), enabled: userOpHash !== '0x', refetchInterval: 1000})

  const createSession = async () => {
    await connectors[0].connect({requests: [{message: 'Sign in'}]})
    // if (walletClient) {
  ...
  const mint = async () => {
    if (address && credentialID) {
      ...
      const submittedUserOpHash = await bundlerClient.sendUserOperation({userOperation: userOpToSubmit})
  ...
  return (
    <div className="bg-blue-700 h-screen w-screen flex flex-col items-center justify-center text-white relative">
      <div className="absolute top-4 right-4">
        {address && (
          <span className="text-lg">{truncateMiddle(address)}</span>
        )}
        {!address && (
          <button
